...
public class VeniceRecordSetProvider
        implements ConnectorRecordSetProvider
{
    private final VeniceConfig config;
    private final VeniceMetadata metadata;

    @Inject
    public VeniceRecordSetProvider(VeniceConfig config, VeniceMetadata metadata)
    {
        this.config = config;
            ...
            List<? extends ColumnHandle> list)
    {
        ...
        List<VeniceColumnHandle> restColumnHandles = list
                .stream()
                .map(c -> (VeniceColumnHandle) c)
                .collect(toList());
        ...
        List<Type> mappedTypes = restColumnHandles
                .stream()
                .map(VeniceColumnHandle::getType)
                .collect(toList());
